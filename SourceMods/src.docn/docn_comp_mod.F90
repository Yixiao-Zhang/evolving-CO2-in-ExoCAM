...
!--------------------------------------------------------------------------
  ...
  character(len=*),parameter :: flds_strm = 'strm_h:strm_qbot:strm_area'
  ...
  character(12),parameter  :: avofld(1:ktrans) = &
     (/ "Si_ifrac    ","Sa_pslv     ","So_duu10n   ","Foxx_taux   ","Foxx_tauy   ", &
        ...
        "So_s        ","Fioo_q      ","strm_h      ","strm_qbot   ","strm_area   "  /)
...
!-------------------------------------------------------------------------------
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    karea = mct_aVect_indexRA(avstrm,'strm_area')
...
!EOP
   ...
   integer :: ierr
   real(R8), parameter :: area_expct = 4*shr_const_pi ! expected total area
   real(R8), parameter :: sst_expct = 290.0 ! kelvin
   real(R8) :: sst_adjust
   integer, parameter :: sum_size = 2
   real(R8), dimension(sum_size) :: local_sum ! [sum of weights, sum of weighted values]
   real(R8), dimension(sum_size) :: world_sum ! the same as above
!-------------------------------------------------------------------------------
...
!   enddo
   ...
   case('SOM')
      ...
      if (firstcall) then
         ...
         local_sum(:) = 0.0
         do n = 1,lsize
            local_sum(1) = local_sum(1) + avstrm%rAttr(karea ,n)
            local_sum(2) = local_sum(2) + avstrm%rAttr(karea ,n)*o2x%rAttr(kt,n)
         enddo
         call mpi_allreduce(local_sum, world_sum, sum_size, mpi_double_precision, mpi_sum, mpicom, ierr)
         ! check
         if (abs(world_sum(1)-area_expct) > 1.0e-4) then
            write(logunit, *) "The sum of area", world_sum(1)
            call shr_sys_abort("The sum of area is not 4pi.")
         endif

         !--- adjust sst, compute ice ---
         sst_adjust = sst_expct - world_sum(2)/world_sum(1)
         if (my_task == master_task) then
            write(logunit, *) "SST adjusted:", sst_adjust
         endif
         do n =1,lsize
            o2x%rAttr(kt,n) = o2x%rAttr(kt,n)+sst_adjust
            ! ice should be turned off
